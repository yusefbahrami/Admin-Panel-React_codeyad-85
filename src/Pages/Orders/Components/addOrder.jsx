...
import { getSingleCartService } from "../../../Services/carts";
  ...
  const handleGetCartsInfo = async (cartId) => {
    ...
    if (res.status === 200) {
      ...
      if (cart.is_ordered) {
        ...
        return Alert("warning", "خطا", "این سبد در سفارش دیگری قرار دارد");
      }
  ...
  const getSelectedOrderInfo = async () => {
    ...
    if (res.status == 200) {
      ...
      setReInitialValues({
        ...
        phone: order.phone,
        email: order.email || "",
        pay_at: order.pay_at ? convertDateToJalaali(order.pay_at) : "",
